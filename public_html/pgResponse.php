...
if($isValidChecksum == "TRUE") {
	if ($_POST["STATUS"] == "TXN_SUCCESS") {
		echo "<div style='text-align:center;font-size:calc(1em + 5vw);padding-top:5vh;padding-bottom:5vh;'>" . "<b>Transaction is Successful</b>". "</div>"  . "<br/>";
		//Process your transaction here as success transaction.
		//Verify amount & order id received from Payment gateway with your application's order id and amount.
	}
	else {echo "<div style='text-align:center;font-size:calc(1em + 5vw);padding-top:5vh;padding-bottom:5vh;'>" . "<b>Transaction is Unsucessful</b>". "</div>" . "<br/>";
		
	}
				echo  '<div class="container">' . '<div class="row justify-content-center align-items-center">' . '<div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-9" style="position:relative;top:-5vh">' ;
	if (isset($_POST) && count($_POST)>0 )
	{			echo '<div class="table-responsive">' . '<table class="table table-dark" style="border-radius:3vh">';
				echo '<thead>';
				echo '</thead>';
				echo '<tbody>';
		foreach($_POST as $paramName => $paramValue) {
				if ($paramName == 'CHECKSUMHASH') continue;
				if ($paramName == 'TXNID') continue;
				if ($paramName == 'MID') continue;
				if ($paramName == 'PAYMENTMODE') continue;
				if ($paramName == 'RESPCODE') continue;
				if ($paramName == 'RESPMSG') continue;
				echo '<tr>' ;
				echo '<td style="text-align:center;font-size:calc(1em + 0.2vw)">' . $paramName . '</td>' ;
				echo '<td style="text-align:center;font-size:calc(1em + 0.2vw);">' . $paramValue . '</td>' ;
				echo '</tr>' ;
